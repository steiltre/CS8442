...
\begin{enumerate}
  ...
  \item
    ...
    \begin{solution}
      ...
      $Assoc(v)$. $Assoc(v)$ will be built on all points contained in $v$'s subtree and will be made to answer grounded queries which are unbounded in
      ...
      $v_{split}$. At $v_{split}$, we know $x \leq x_{v_{split}} < x'$, where $x_v$ is the $x$-value stored at $v$. Therefore, all leaves in the
      ...
      $x$-coordinates of points stored in $lc({v_{split}})$ and $rc({v_{split}})$.
